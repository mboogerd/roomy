...
export class Room {
  ...
  private async maybeTick() {
    ...
    try {
      ...
      const ops = restructuring
        ? await llm.restructure(this.state, this.summary)
        : await llm.tick(this.state, window, this.summary);
      this.commit(ops);

      if (this.ticks % SUMMARY_EVERY === 0 && this.sinceSummary.length) {
        this.summary = await llm.summarize(this.summary, this.sinceSummary);
        this.sinceSummary = [];
      }
    } catch (err) {
  ...
  async renderFailed(id: string, error: string) {
    ...
    if (!block) return;
    try {
      const ops = await llm.repair(id, block.source, error);
